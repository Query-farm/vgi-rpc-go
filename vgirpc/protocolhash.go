...
// what is on the wire, and folding the framework versions in would rotate
// every protocol's hash on a release that changed no protocol. The "v1"
// domain tag is the only version the hash carries, and it moves only when the
// hash *definition* moves.
//
...
//
// It still reaches clients as stream_kind on the description, where "unknown"
// is a sayable answer. A hash has no such option -- which is the whole
// difference: a description may admit what it does not know, a fingerprint
// may not.
